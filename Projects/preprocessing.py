...
class Preprocessing:
    ...
    def extract_windows(self, window_size, overlap):
        ...
        for start in range(0, len(self.record.p_signal) - window_size + 1, step_size):
            ...
            annotation_symbols_in_window = [
                self.annotation.symbol[i]
                for i in range(len(self.annotation.sample))
                if self.annotation.sample[i] in annotations_in_window
            ]
    ...
    def balance_data(self):
        ...
        signal1_balanced, labels_balanced = smote.fit_resample(
            signal1_2d,
            self.labels.train,
        )
    ...
    def normalize_minmax(self):
        ...
        self.signal1.train = scaler_signal1.fit_transform(
            self.signal1.train.reshape(-1, self.signal1.train.shape[-1]),
        ).reshape(self.signal1.train.shape)
        self.signal1.val = scaler_signal1.transform(
            self.signal1.val.reshape(-1, self.signal1.val.shape[-1]),
        ).reshape(self.signal1.val.shape)
        self.signal1.test = scaler_signal1.transform(
            self.signal1.test.reshape(-1, self.signal1.test.shape[-1]),
        ).reshape(self.signal1.test.shape)

        self.signal2.train = scaler_signal2.fit_transform(
            self.signal2.train.reshape(-1, self.signal2.train.shape[-1]),
        ).reshape(self.signal2.train.shape)
        self.signal2.val = scaler_signal2.transform(
            self.signal2.val.reshape(-1, self.signal2.val.shape[-1]),
        ).reshape(self.signal2.val.shape)
        self.signal2.test = scaler_signal2.transform(
            self.signal2.test.reshape(-1, self.signal2.test.shape[-1]),
        ).reshape(self.signal2.test.shape)
